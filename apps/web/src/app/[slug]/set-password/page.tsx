...
export default function SetPasswordPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const setupMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(`/api/partners/${slug}/auth`, {
        ...
        body: JSON.stringify({ email, password, setup_token: token }),
      });
      if (!res.ok) {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#F7F4EF] px-6">
        <div className="text-center">
          ...
          <h2 className="text-lg font-black text-gray-900">Invalid Setup Link</h2>
          <p className="mt-2 text-sm text-gray-500">
            This link is missing required information. Try signing in again to receive a new email.
          </p>
          <Link
            href={`/${slug}/login`}
          ...
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            {done ? (
              <div className="px-6 py-8 text-center">
                ...
                <h2 className="mb-2 text-base font-black text-gray-900">Password Set!</h2>
              ...
              <div className="p-6">
                <p className="mb-5 text-sm text-gray-500">
                  Setting up account for <strong className="text-gray-700">{email}</strong>
                </p>
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    disabled={!password || !confirm || setupMutation.isPending}
