...
- Drizzle client at `apps/web/src/db/index.ts` (neon-serverless Pool: `max: 5`, `idleTimeoutMillis: 20_000` so idle WS connections drop and Neon can suspend); config at `apps/web/drizzle.config.ts`; schemas in `apps/web/src/db/schema/`, queries in `apps/web/src/db/queries/`.
